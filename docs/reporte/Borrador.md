...
Para definir genes diferencialmente expresados se utilizó un umbral de FDR < 0.01 y |log2FoldChange| > 1. El FDR permitió controlar la tasa de falsos descubrimientos, mientras que el umbral de log2FoldChange permitió seleccionar genes con cambios de expresión de mayor magnitud biológica. En particular, un |log2FoldChange| > 1 corresponde a un cambio mínimo de dos veces entre condiciones, por lo que este criterio evita enfocarse en genes con diferencias estadísticamente significativas pero de efecto pequeño. Esto es especialmente relevante en una comparación entre E11.5 y E14.5, donde se espera una señal transcripcional amplia debido a la transición del telencéfalo desde una etapa de proliferación de células madre neurales hacia una etapa de neurogénesis activa.

Elegí |log2FC| > 1 porque representa un cambio de al menos dos veces en expresión. Como la comparación entre E11.5 y E14.5 genera muchos genes significativos, este filtro me permite priorizar genes con cambios más grandes y más interpretables biológicamente, no solo genes con padj bajo.